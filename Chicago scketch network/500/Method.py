...
class Solve:
    ...
    def g_solving_the_charging_location_problem_by_LR(self):
        ...
        for i in range(self.iteration_times):
            ...
            for od_index in range(self.g_number_of_ODs):
                ...
                if self.g_ending_state_vector.state_vector[0].Primal_label_cost==0:
                    self.accessible_Flag[i].append(1)
                else:
                    self.accessible_Flag[i].append(0)
            #3. update multiplier
            ...
            for j in range(self.g_number_of_charging_stations):
                ...
                for k in range(self.g_number_of_ODs):
                    traveling_flag=candidate_travel_flag_list[k]
                    #output
                    self.record_multiplier_miu[i].append(copy.copy(node.base_profit_for_lagrangian))
    ...
    def output_results(self,spend_time):
        ...
        with open("output_solution_of_routing_subproblems.csv","w") as fl:
        ...
        with open("OD_pair_accessible_flag.csv","w") as fl:
            fl.write("Iteration,ID,From_node,To_node,Flag\n")
            for i in range(len(self.global_UB)):
                for k in range(self.g_number_of_ODs):
                    OD_pair = self.OD_pair_list[k]
                    from_node = OD_pair[0]
                    to_node = OD_pair[1]
                    flag = self.accessible_Flag[i][k]
                    fl.write(str(i + 1) + "," + str(k + 1) + "," + str(from_node) + "," + str(to_node) + "," + str(flag) + "\n")
